...
fn test_sale_with_fail_add_trade_storage(){
    ...
    darmaji.call(
        ...
        &json!({
            ...
            "account_id": marketplace.account_id(),
            "msg": &json!{{
                "market_type": "accept_trade_zonulet_series",
...
#[test]
fn test_accept_offer_zonulet_series() {
    let (marketplace, nft, _, alice, bob, chandra, darmaji, _) = init();
    ...
    // chandra accept bid and nft_approve to marketplace
    let msg = &json!(
        {"market_type":"accept_offer_zonulet_series","buyer_id": bob.account_id, "price": U128(10u128.pow(24))}).to_string();
...
fn test_accept_offer_series_deleted_offer_data() {
    ...
    // should be updated chandra's approval id
    let msg = &json!(
        {"market_type":"accept_offer_zonulet_series","buyer_id": bob.account_id, "price": U128(10u128.pow(24))})
    .to_string();
...
#[test]
fn test_accept_trade_zonulet_series(){
    let (marketplace, nft, _, alice, bob, chandra, darmaji, _) = init();
    create_nft_and_mint_one(&nft, &alice, &bob, &chandra, &darmaji);
