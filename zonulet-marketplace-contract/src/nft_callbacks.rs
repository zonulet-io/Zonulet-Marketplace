...
impl NonFungibleTokenApprovalsReceiver for Contract {
    fn nft_on_approve(
        ...
        assert_ne!(
            ...
            "Zonulet: nft_on_approve should only be called via cross-contract call"
        );
        assert_eq!(owner_id, signer_id, "Zonulet: owner_id should be signer_id");
        ...
        if market_type == "sale" {
            assert!(price.is_some(), "Zonulet: price not specified");
            ...
            if self.approved_ft_token_ids.contains(&ft_token_id_res) != true {
                env::panic_str(&"Zonulet: ft_token_id not approved");
            }
            ...
            assert!(buyer_id.is_some(), "Zonulet: Account id is not specified");
            assert!(price.is_some(), "Zonulet: Price is not specified (for check)");
        ...
        } else if market_type == "accept_offer_zonulet_series" {
            assert!(buyer_id.is_some(), "Zonulet: Account id is not specified");
            assert!(
                self.zonulet_nft_contracts.contains(&nft_contract_id),
                "Zonulet: accepting offer series for Zonulet NFT only"
            );
            assert!(price.is_some(), "Zonulet: Price is not specified (for check)");
            ...
            assert!(buyer_id.is_some(), "Zonulet: Account id is not specified");
            assert!(buyer_nft_contract_id.is_some(), "Zonulet: Buyer NFT contract id is not specified");
            assert!(buyer_token_id.is_some(), "Zonulet: Buyer token id is not specified");
        ...
        } else if market_type == "accept_trade_zonulet_series" {
