...
pub struct ContractV2 {
    ...
    pub zonulet_nft_contracts: UnorderedSet<AccountId>,
...
pub enum StorageKey {
    ...
    Offers,
    ZonuletNFTContractIds,
    MarketV2,
    MarketV3,
    OffersV2,
    ZonuletNFTContractIdsV2,
    Trade,
...
impl Contract {
    ...
    pub fn new(
        ...
        zonulet_nft_contracts: Option<Vec<AccountId>>,
        current_fee: u16,
    ) -> Self {
        assert!(current_fee < MAX_TREASURY_PERCENTAGE, "Zonulet: fee is higher than {}", MAX_TREASURY_PERCENTAGE);
        ...
        let mut this = Self {
            ...
            zonulet_nft_contracts: UnorderedSet::new(StorageKey::ZonuletNFTContractIds),
        ...
        add_accounts(zonulet_nft_contracts, &mut this.zonulet_nft_contracts);
    ...
    pub fn migrate() -> Self {
        ...
        let this = Contract {
            ...
            offers: prev.offers,
            zonulet_nft_contracts: prev.zonulet_nft_contracts,
    ...
    pub fn set_transaction_fee(&mut self, next_fee: u16, start_time: Option<TimestampSec>) {
        ...
        assert!(next_fee < MAX_TREASURY_PERCENTAGE, "Zonulet: fee is higher than {}", MAX_TREASURY_PERCENTAGE);
    ...
    // Approved zonulet contracts
    #[payable]
    pub fn add_approved_zonulet_nft_contract_ids(&mut self, nft_contract_ids: Vec<AccountId>) {
        self.assert_owner();
        add_accounts(Some(nft_contract_ids), &mut self.zonulet_nft_contracts);
    }
    ...
    pub fn buy(
        ...
        let market_data: Option<MarketData> =
            if let Some(market_data) = self.old_market.get(&contract_and_token_id) {
                ...
                env::panic_str(&"Zonulet: Market data does not exist");
            };

        let market_data: MarketData = market_data.expect("Zonulet: Market data does not exist");
        ...
        assert_ne!(
            ...
            "Zonulet: Cannot buy your own sale"
        );
        ...
        assert_eq!(
            ...
            NEAR,
            "Zonulet: NEAR support only"
        );

        assert_eq!(is_auction, false, "Zonulet: the NFT is on auction");
        ...
        assert_eq!(
            ...
            "Zonulet: The attached deposit should be exactly the price {}",
            price
        );
    ...
    fn internal_process_purchase(
        ...
        let market_data = self
            ...
            .expect("Zonulet: Sale does not exist");
    ...
    pub fn add_offer(
        ...
        let token = if token_id.is_some() {
            ...
            assert!(
                self.zonulet_nft_contracts.contains(&nft_contract_id),
                "Zonulet: offer series for Zonulet NFT only"
            );
            token_series_id.as_ref().unwrap().to_string()
        };
        ...
        assert_eq!(
            ...
            price.0,
            "Zonulet: Attached deposit != price"
        );

        assert_eq!(
            ft_token_id.to_string(),
            "near",
            "Zonulet: Only NEAR is supported"
        );

        assert!(
            self.approved_nft_contract_ids.contains(&nft_contract_id),
            "Zonulet: nft_contract_id is not approved"
        );
    ...
    pub fn delete_offer(
        ...
        let offer_data = self
            ...
            .expect("Zonulet: Offer does not exist");
        ...
        assert_eq!(
            ...
            "Zonulet: Caller not offer's buyer"
        );
        ...
        .expect("Zonulet: Offer not found");
    ...
    fn internal_accept_offer(
        ...
        if offer_data_raw.is_none() {
            ...
            env::log_str("Zonulet: Offer does not exist");
    ...
    fn internal_accept_offer_series(
        ...
        // Token delimiter : is specific for Zonulet NFT
    ...
    fn internal_add_trade(
        ...
        let token = if token_id.is_some() {
            ...
            assert!(
                self.zonulet_nft_contracts.contains(&nft_contract_id),
                "Zonulet: trade series for Zonulet NFT only"
            );
            token_series_id.as_ref().unwrap().to_string()
        };
        ...
        let buyer_contract_account_id_token_id = make_triple(
            ...
            &buyer_token_id
                .as_ref()
                .expect("Zonulet: Buyer token id is not specified"),
        );
    ...
    pub fn delete_trade(
        ...
        let trade_list = self
            .trades
            .get(&buyer_contract_account_id_token_id)
            .expect("Zonulet: Trade list does not exist");

        let trade_data = trade_list
            .trade_data
            .get(&contract_account_id_token_id)
            .expect("Zonulet: Trade data does not exist");
        ...
        .expect("Zonulet: Trade not found");
    ...
    fn internal_delete_trade(
        ...
        let mut by_owner_id = self
            .by_owner_id
            .get(&buyer_id)
            .expect("Zonulet: no market data by account_id");
    ...
    pub fn callback_first_trade(
        ...
        if !is_promise_success() {
            env::panic_str(&"Zonulet: buyer's nft failed to trade");
        } else {
    ...
    pub fn callback_second_trade(
        ...
        if !is_promise_success() {
            ...
            env::panic_str(&"Zonulet: seller's nft failed to trade, rollback buyer's nft");
        } else {
    ...
    pub fn add_bid(
        ...
        let mut market_data = self
            ...
            .expect("Zonulet: Token id does not exist");

        assert_eq!(market_data.is_auction.unwrap(), true, "Zonulet: not auction");
        ...
        assert!(
            ...
            "Zonulet: Sale has not started yet"
        );

        assert!(
            current_time <= market_data.ended_at.unwrap(),
            "Zonulet: Sale has ended"
        );
        ...
        assert_ne!(market_data.owner_id, bidder_id, "Zonulet: Owner cannot bid their own token");

        assert!(
            env::attached_deposit() >= amount.into(),
            "Zonulet: attached deposit is less than amount"
        );

        assert_eq!(ft_token_id.to_string(), "near", "Zonulet: Only support NEAR");
        ...
        if !bids.is_empty() {
            ...
            assert!(
              ...
              "Zonulet: Can't pay less than or equal to current bid price + 5% : {:?}",
              current_bid.price.0 + (current_bid.price.0 * 5 / 100)
            );

            assert!(
                amount.0 >= market_data.price,
                "Zonulet: Can't pay less than starting price: {:?}",
    ...
    fn internal_cancel_bid(&mut self, nft_contract_id: AccountId, token_id: TokenId, account_id: AccountId) {
      ...
      assert!(
        !bids.is_empty(),
        "Zonulet: Bids data does not exist"
      );
    ...
    pub fn accept_bid(&mut self, nft_contract_id: AccountId, token_id: TokenId) {
        ...
        assert!(
            ...
            "Zonulet: Seller or owner only"
        );

        if predecessor_account_id == self.owner_id && market_data.ended_at.is_some() {
          assert!(
            current_time >= market_data.ended_at.unwrap(),
            "Zonulet: Auction has not ended yet"
          );
        }

        let mut bids = market_data.bids.unwrap();

        assert!(!bids.is_empty(), "Zonulet: Cannot accept bid with empty bid");
    ...
    pub fn end_auction(&mut self, nft_contract_id: AccountId, token_id: TokenId) {
      ...
      let mut market_data = self
          .market
          .get(&contract_and_token_id)
          .expect("Zonulet: Market data does not exist");
      ...
      if predecessor_account_id == self.owner_id && market_data.ended_at.is_some() {
        assert!(
          current_time >= market_data.ended_at.unwrap(),
          "Zonulet: Auction has not ended yet (for owner)"
        );
      }
        ...
        if let Some(is_auction) = is_auction {
          if let Some(started_at) = started_at {
            ...
            if is_auction == true {
                ...
                assert!(ended_at.is_some(), "Zonulet: Ended at is none");
    ...
    pub fn get_market_data(&self, nft_contract_id: AccountId, token_id: TokenId) -> MarketDataJson {
        ...
        let market_data = market_data.expect("Zonulet: Market data does not exist");
    ...
    fn internal_transfer_near(&self, account_id: AccountId, amount: Balance){
        ...
        if balance < amount {
            env::panic_str(&format!("Zonulet: Not enough balance to transfer near, balance: {}, amount: {}", balance, amount));
        }
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "Zonulet: Owner only")]
    ...
    #[test]
    #[should_panic(expected = "Zonulet: price higher than 1000000000000000000000000000000000")]
    ...
    #[test]
    #[should_panic(expected = "Zonulet: Market data does not exist")]
    ...
    #[test]
    #[should_panic(expected = "Zonulet: Offer does not exist")]
    ...
    #[test]
    #[should_panic(expected = "Zonulet: Trade list does not exist")]
    ...
    #[test]
    #[should_panic(expected = "Zonulet: the NFT is on auction")]
